...
def get_parser():
    parser = argparse.ArgumentParser(
        description="extract intermediate layer's features of wav2vec/data2vec models"
    )
    parser.add_argument('--data-dir', help='location of tsv files')
...
def get_iterator(args):
    with open(osp.join(args.data_dir, args.split) + ".tsv", "r") as fp:
        lines = fp.read().split("\n")
...
def main():
    ...
    def create_files(dest):
        copyfile(osp.join(args.data_dir, args.split) + ".tsv", dest + ".tsv")
